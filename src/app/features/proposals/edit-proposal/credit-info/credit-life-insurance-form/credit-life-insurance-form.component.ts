...
@Component({
  selector: 'app-credit-life-insurance-form',
  standalone: true,
  imports: [
    CommonModule, 
    FormsModule, 
  ...
  templateUrl: './credit-life-insurance-form.component.html',
  styles: [`
...
export class CreditLifeInsuranceFormComponent implements OnInit {
  ...
  hasLoan = linkedSignal(() => this.record()?.has_loan ?? false);
  loanRemainingAmount = linkedSignal(() => Number(this.record()?.loan_remaining_amount) || 0);
  ...
  save() {
    ...
    const payload = {
      ...
      loan_remaining_amount: this.loanRemainingAmount(),
      will_assign_policy: this.willAssignPolicy(),
      non_assignment_reason: this.nonAssignmentReason()
    };

    this.proposalsService.updateLifeInsurance(this.proposalId, payload).subscribe({
      next: (res) => {
        this.loading.set(false);
        this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Life insurance policy saved' });
