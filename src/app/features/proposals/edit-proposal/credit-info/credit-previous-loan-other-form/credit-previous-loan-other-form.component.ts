...
@Component({
  selector: 'app-credit-previous-loan-other-form',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './credit-previous-loan-other-form.component.html',
  styles: [`
...
export class CreditPreviousLoanOtherFormComponent implements OnInit {
  ...
  // Field Signals (Manual Typing as requested)
  ...
  save() {
    ...
    this.proposalsService.updatePreviousLoanOtherBank(this.proposalId, payload).subscribe({
      next: (res) => {
        this.loading.set(false);
        this.messageService.add({ severity: 'success', summary: 'Success', detail: 'External historical loan saved' });
        this.ref.close({ saved: true, data: res.data });
      },
      error: (err) => {
        this.loading.set(false);
        this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to save historical loan' });
      }
    });
  }
