...
@Component({
  selector: 'app-credit-other-guarantee-form',
  standalone: true,
  imports: [
    CommonModule, 
    FormsModule, 
  ...
  templateUrl: './credit-other-guarantee-form.component.html',
  styles: [`
...
export class CreditOtherGuaranteeFormComponent implements OnInit {
  ...
  save() {
    ...
    this.proposalsService.updateCreditGuaranteeOtherBank(this.proposalId, payload).subscribe({
      next: (res) => {
        this.loading.set(false);
        this.messageService.add({ severity: 'success', summary: 'Success', detail: 'External guarantee info saved successfully' });
        this.ref.close({ saved: true, data: res.data });
      },
      error: (err) => {
        this.loading.set(false);
        this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to save external guarantee info' });
      }
    });
  }
