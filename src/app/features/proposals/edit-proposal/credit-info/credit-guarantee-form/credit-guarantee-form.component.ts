...
@Component({
  selector: 'app-credit-guarantee-form',
  standalone: true,
  imports: [
    CommonModule, 
    FormsModule, 
  ...
  templateUrl: './credit-guarantee-form.component.html',
  styles: [`
    .guarantee-form-container { overflow-x: hidden; }
...
export class CreditGuaranteeFormComponent implements OnInit {
  ...
  borrowerName = linkedSignal(() => this.record()?.borrower_name || '');
  ...
  save() {
    ...
    this.proposalsService.updateCreditGuaranteeThisBank(this.proposalId, payload).subscribe({
      next: (res) => {
        this.loading.set(false);
        this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Guarantee info saved successfully' });
        this.ref.close({ saved: true, data: res.data });
      },
      error: (err) => {
        this.loading.set(false);
        this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Failed to save guarantee info' });
      }
    });
  }
