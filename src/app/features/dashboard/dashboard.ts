...
export class Dashboard implements OnInit {
  ...
  ngOnInit() {
    ...
    setTimeout(() => {
      if (this.userTypeId === 1 || this.userTypeId === 9 || this.userTypeId === 5) {
  ...
  getUserRoleLabel(typeId: number): string {
    switch (typeId) {
      ...
      case 3:
        return 'Compliance Officer / Employee';
      case 4:
  ...
  loadAuthorizedUnits() {
    ...
    this.service.findAll({ employee_id: this.employeeId }).subscribe({
      next: (res: any) => {
        ...
        if (mapped.length > 0) {
          this.unitsList = [{ label: 'All Branches', value: 0 }, ...mapped];
          this.selectedUnitId = 0;
          this.loadUnitDashboard(0);
        } else {
          this.unitsList = [];
  ...
  onPeriodChange(periodId: string) {
    ...
    if (this.selectedUnitId !== null && this.selectedUnitId !== undefined) {
  ...
  get filteredAssessmentPeriods() {
    ...
    const selectedYear = (this.unitSummary?.yearWiseAssessments || []).find(
      (y: any) => Number(y.year_id) === Number(this.selectedYearId),
    );
    ...
    const scheduledAssessmentIds = (selectedYear.assessments || []).map((ass: any) =>
      Number(ass.id),
    );

    const allOption = periods.find((p) => p.value === 'all');
    const filteredRest = periods.filter(
      (p: any) => p.value !== 'all' && scheduledAssessmentIds.includes(Number(p.value)),
    );
  ...
  getAuditStatusLabel(statusId: number): string {
    switch (statusId) {
      ...
      case 2:
        return 'AUDIT REVIEW PENDING';
      case 3:
        return 'RE-EDIT REQUEST';
      case 4:
